...
static int _ReadSectors(U8 Unit, U32 SectorIndex, void * pBuffer, U32 NumSectors) {
  ...
  Status = USBH_MSD_ReadSectors(Unit, SectorIndex, NumSectors, (U8 *)pBuffer);
  if (Status == USBH_STATUS_DEVICE_REMOVED) {
    ...
    pDriverName = _GetDriverName(Unit);
    Len         = strlen(pDriverName);

    memcpy(acVolume, pDriverName, Len);
    acVolume[Len++] = ':';
    acVolume[Len++] = '0' + Unit;
    acVolume[Len++] = ':';
    acVolume[Len]   = '\0';
    
    pVolume = FS_FindVolume(acVolume);
...
static int _WriteSectors(U8 Unit, U32 SectorIndex, const void * pBuffer, U32 NumSectors, U8 RepeatSame) {
  ...
  if (RepeatSame) {
    do {
      Status = USBH_MSD_WriteSectors(Unit, SectorIndex++, 1, (const U8 *)pBuffer);
      if (Status) {
        goto OnError;
      }
    } while (--NumSectors);
  } else {
    Status = USBH_MSD_WriteSectors(Unit, SectorIndex, NumSectors, (const U8 *)pBuffer);
    if (Status) {
      goto OnError;
    }
  }
