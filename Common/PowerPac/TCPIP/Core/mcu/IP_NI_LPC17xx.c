...
*
*  Function description:
*    Allocates the Rx and Tx buffer descriptors
*
*  Notes
*    (1) Buffer descriptors MUST start on a 32-byte boundary.
*/
static void _AllocBufferDescs(void) {
  ...
  pMem        += NUM_RX_BUFFERS * sizeof(RX_BUFFER_DESC);
  _paRxBufStat = (RX_BUFFER_STAT *)pMem;
  pMem        += NUM_RX_BUFFERS * sizeof(RX_BUFFER_STAT);
  _pRxBuffer   = pMem;
  pMem        += NUM_RX_BUFFERS * RX_BUFFER_SIZE;
  //
  // Assign memory to TX descriptors, status blocks and buffers
  //
  _paTxBufDesc = (TX_BUFFER_DESC *)pMem;
  pMem        += NUM_TX_BUFFERS * sizeof(TX_BUFFER_DESC);
  _paTxBufStat = (TX_BUFFER_STAT *)pMem;
  pMem        += NUM_TX_BUFFERS * sizeof(TX_BUFFER_STAT);
  _pTxBuffer   = pMem;
}
...
static  int  _Init(unsigned Unit) {
  ...
  MAC_RXDESCRIPTORNUM = NUM_RX_BUFFERS -1;
  MAC_TXDESCRIPTOR    = (U32) _paTxBufDesc;
  MAC_TXSTATUS        = (U32) _paTxBufStat;
  MAC_TXDESCRIPTORNUM = NUM_TX_BUFFERS -1;
  //
  // Enable Rx & Tx
  //
  MAC_MAC1 |= (1 << 0);
  ...
  // Enable interrupts
  //
  BSP_ETH_InstallISR_Ex(ISR_ID, _ISR_Handler, ISR_PRIO);
...
**********************************************************************
*/
const IP_HW_DRIVER IP_Driver_LPC17xx = {
  _Init,
  _SendPacketIfTxIdle,
  _GetPacketSize,
  _ReadPacket,
