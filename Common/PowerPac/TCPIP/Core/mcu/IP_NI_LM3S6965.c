...
**********************************************************************
----------------------------------------------------------------------
File    : IP_NI_LM3S6965.c
Purpose : Network interface driver for the LM3S6965
--------  END-OF-HEADER  ---------------------------------------------
*/

#include "IP_Int.h"
#include "BSP.h"
#include "IP_NI_LM3S6965.h"
...
static void _ISR_Handler(void) {
  ...
  if (ISR_Status & MAC_RXER) {
    ...
    MACIM  &= ~(1 << 0);  // Disable Rx interrupt
    IP_OnRx();
  }
...
static  int  _Init(unsigned Unit) {
  ...
  BSP_ETH_Init(Unit);
  //
  // GPIO Port F for Ethernet LEDs
  //
  r = GPIO_PORTF_DIR;
  r &= ~((1 << 3) | (1 << 2));
  GPIO_PORTF_DIR = r;
  GPIO_PORTF_AFSEL |=  ((1 << 3) | (1 << 2));
  GPIO_PORTF_DR2R  |=  ((1 << 3) | (1 << 2));
  GPIO_PORTF_DEN   |=  ((1 << 3) | (1 << 2));
...
**********************************************************************
*/
const IP_HW_DRIVER IP_Driver_LM3S6965 = {
  _Init,
  _SendPacketIfTxIdle,
  _GetPacketSize,
  _ReadPacket,
