...
**********************************************************************
----------------------------------------------------------------------
File    : IP_NI_AT91SAM9XE.c
Purpose : NI driver for AT91SAM9XE
          Driver defaults to use the built-in SRAM for Tx buffers,
          Tx descriptors and Rx descriptors. SDRAM is only used for
          Rx buffers. The Tx buffer and buffer descriptors should
          always remain in SRAM.
...
#define NUM_RX_BUFFERS       (16 * 12 + 1)  // One buffer is 128 bytes.
                                            // 12 is enough for 1 large packets.
                                            // One extra to allow n large packets.

#define USE_RMII                       (1)  // Use RMII interface to PHY
#define RX_BUFFERS_LOCATED_IN_SDRAM    (1)  // Use RMII interface to PHY

#define TX_BUFFER_SIZE              (1536)
#define TX_MEM_START_ADDR       (0x307000)  // Internal SRAM, physical & virtual address
#define TX_MEM_SIZE               (0x1000)  // Internal SRAM, we use of the internal SRAM 4KB
...
/*********************************************************************
*
*       AT91SAM9XE peripheral addresses, sfrs and related control bits
*/
...
#define AIC_SMR                       (*(volatile U32*)(AIC_BASE_ADDR + 0x000)) // (AIC) Source Mode Register
#define AIC_SVR                       (*(volatile U32*)(AIC_BASE_ADDR + 0x080)) // (AIC) Source Vector Register
#define AIC_IECR                      (*(volatile U32*)(AIC_BASE_ADDR + 0x120)) // (AIC) Interrupt Enable Command Register
#define AIC_IDCR                      (*(volatile U32*)(AIC_BASE_ADDR + 0x124)) // (AIC) Interrupt Disable Command Register
#define AIC_ICCR                      (*(volatile U32*)(AIC_BASE_ADDR + 0x128)) // (AIC) Interrupt Clear Command Register
...
//
// Peripheral ID definitions for AT91SAM9XE
//
#define ID_PIOA                       ((unsigned int)  2)
#define ID_PIOB                       ((unsigned int)  3)
#define ID_PIOC                       ((unsigned int)  4)
#define ID_EMAC                       ((unsigned int) 21) // Ethernet MAC

//
// EMAC peripheral
//
#define EMAC_BASE_ADDR                (0xFFFC4000)
...
#define EMAC_DTF                      (*(volatile U32 *)(EMAC_BASE_ADDR + 0x058)) // (EMAC) Deferred Transmission FrameRegister
...
static void*         _pTxBuffer     = (void*) TX_MEM_START_ADDR;
static BUFFER_DESC*  _pTxBufferDesc = (BUFFER_DESC*) ((TX_MEM_START_ADDR + TX_BUFFER_SIZE + 7) &~0x07);
...
static void _AllocBufferDescs(void) {
  ...
  //
  _paRxBufDesc = _pTxBufferDesc + NUM_TX_BUFFERS;
  if ((U32)(_paRxBufDesc + _NumRxBuffers) > (TX_MEM_START_ADDR + TX_MEM_SIZE)) {
...
static int  _SendPacket(void) {
  ...
  //
  // We always copy into local tx buffer, to avoid matrix arbitration problem in AT91SAM9XE
  //
...
static  int  _Init(unsigned Unit) {
  ...
  _PHY_Context.UseRMII = USE_RMII;         // Use RMII
  ...
  EMAC_NCFGR  = (0 << 8)                   // 1: BIG: Allow packets of up to 1536 bytes
              ...
              ;
  //
  // Perform hardware reset using RESET pin of MCU; PIO for communication to PHY should be initialized before.
  //
  RSTC_RMR = 0xA5000000 | RSTC_ERSTL & (1 << 8);
  RSTC_RCR = 0xA5000000 | RSTC_EXTRST;
  while ((RSTC_RSR & RSTC_NRSTL) == 0);       // Wait until RESET timer has expired (just a few microseconds)
  _PHY_pDriver->pfInit(&_PHY_Context);        // Configure the PHY
  if (_PHY_Context.UseRMII) {    // 0: MII, 1: RMII
    EMAC_USRIO |= (1 << 0)       // Enable RMII mode,
               |  (1 << 1)       // Enable clock for interface to Phy
               ;
  } else {
    EMAC_USRIO &= ~(1 << 0);     // Clear RMII mode
    EMAC_USRIO |=  (1 << 1);     // Enable clock for interface to Phy
  }
  //
...
/*********************************************************************
*
*       IP_Driver_SAM9XE_ConfigNumRxBuffers
*
*  Function description
...
**********************************************************************
*/
const IP_HW_DRIVER IP_Driver_SAM9XE = {
  _Init,
  _SendPacketIfTxIdle,
  _GetPacketSize,
  _ReadPacket,
