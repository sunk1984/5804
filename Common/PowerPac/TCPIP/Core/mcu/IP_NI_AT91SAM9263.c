...
**********************************************************************
----------------------------------------------------------------------
File    : IP_NI_AT91SAM9263.c
Purpose : NI driver for AT91SAM9263
          Driver defaults to use the built-in SRAM0 for Tx buffers,
          Tx descriptors and Rx descriptors. SDRAM is only used for
          Rx buffers. The descriptors should remain in SRAM0. This
...
#else
  #define USE_RMII           (0)           // Use MII interface to PHY
#endif

#define TX_BUFFER_SIZE     (1536)
#define TX_MEM_START_ADDR  (0x500000)      // Internal SRAM0, original address
#define TX_MEM_SIZE        (0x4000)        // Internal SRAM0 is 16KB
...
/*********************************************************************
*
*       AT91SAM9263 peripheral addresses, sfrs and related control bits
*/
//
...
static int  _SendPacket(void) {
  ...
  //
  // We always copy into local tx buffer, to avoid matrix arbitration problem in AT91SAM9263
  //
...
/*********************************************************************
*
*       IP_Driver_SAM9263_ConfigNumRxBuffers
*
*  Function description
*    Sets the number of Rx Buffers in the config phase.
*/
void IP_NI_SAM9263_ConfigNumRxBuffers(U16 NumRxBuffers) {
  if (_IsInited == 0) {
...
**********************************************************************
*/
const IP_HW_DRIVER IP_Driver_SAM9263 = {
  _Init,
  _SendPacketIfTxIdle,
  _GetPacketSize,
  _ReadPacket,
